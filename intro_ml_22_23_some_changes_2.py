# -*- coding: utf-8 -*-
"""intro_ml_22_23_some_changes.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1CIoFQmhHgo6IQz9HCmnfWhYo08JRUg-A
...
class kNNClassifier:
  ...
  def fit(self, X, y):
    self.X = X
    self.y = y
      #np.column_stack((X, y.reshape(-1, 1))) concatenate
  def predict(self, X):
...
interactive_plot = interactive(decision_plane, k=(1,100,1))
interactive_plot
...
plt.title('Test vs Validation Error for Different k values')
plt.legend()
plt.show()

optimal_k = k_values[valid_errors.index(min(valid_errors))]
print(f"Optimal k (using validation sample) is : {optimal_k}", )
...
Недообучение обычно происходит при очень малых значениях k, когда модель слишком проста, чтобы уловить сложность данных. На вашем графике это могло бы соответствовать начальной части графика (при малых значениях k), где обе ошибки высоки. Однако, на представленном графике при малых значениях k ошибка на обучающем наборе низкая, что не характерно для недообучения.

Переобучение (Overfitting):
Переобучение возникает, когда модель слишком сложна и начинает запоминать шум или особенности обучающего набора, что приводит к плохой обобщающей способности. На графике это можно увидеть как разрыв между ошибками на обучающем и валидационном наборах. Когда k увеличивается, ошибка на обучающем наборе постепенно возрастает, что указывает на уменьшение переобучения. Тем не менее, в точке, где ошибка на валидационном наборе начинает стабилизироваться или увеличиваться, в то время как ошибка на обучающем наборе все еще снижается или остается относительно низкой, мы можем начать наблюдать переобучение. Это обычно происходит на правом конце графика при больших значениях k.
"""
...
print(f"Optimal k (using test sampe) is : {optimal_k}", )
...
#round(np.mean(np.array(k_values_model)))
# Я так понимаю, что это не правильно (усреднять модель), надо усреднять ответы из этих моделей... Думаю, что лучшу убрать,в задание не сказано усреднить...
